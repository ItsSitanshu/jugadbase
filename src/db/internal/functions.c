...
/* TODO: argument based tuples.
 e.g. SIN(machine_1.angle) will require seperate tuple from SIN(machine_2.angle)
*/
...
ColumnValue fn_radians(ExprNode** args, uint8_t arg_count, Row* row, Database* db) {
  ...
  bool valid_conversion = infer_and_cast_value_raw(&input, TOK_T_DOUBLE);
  if (!valid_conversion) {
    LOG_ERROR("Invalid conversion whilst trying to insert row %s -> %s", token_type_strings[TOK_T_DOUBLE], token_type_strings[input.type]);
    return result;
  }
